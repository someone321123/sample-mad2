<template>
  <div class="container">
    ...
    <div v-if="adRequest">
      ...
      <div v-if="adRequest.negotiation_status === 'pending'">
        <div class="form-group">
          <label for="negotiationAmount">Propose New Amount:</label>
          <input type="number" id="negotiationAmount" v-model.number="negotiationAmount" class="form-control" min="0" />
        </div>
        <button type="button" class="btn btn-primary" @click="negotiateAdRequest">Propose</button>
      </div>
...
const negotiateAdRequest = async () => {
  try {
    const token = localStorage.getItem("token");
    const response = await axios.post(`/influencer/negoAdRequest/${adRequestId}`, new URLSearchParams({
